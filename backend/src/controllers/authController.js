...
exports.register = async (req, res) => {
    try {
        ...
        const user = await User.create({ name, email, password: hash, gender, phone });
        await Patient.create({ user_id: user.id, name: user.name, gender: user.gender, relation: "self" });
        ...
        res.json({ user: { id: user.id, name: user.name, email: user.email, role: user.role, gender: user.gender, phone: user.phone }, token });
    } catch (err) {
